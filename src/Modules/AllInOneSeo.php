            ...
            'admin-bar' => [
                ...
                'register' => static function (): void {
                    add_action('admin_bar_menu', static function (WP_Admin_Bar $bar): void {
                        ...
                        // The gear logo is a baked-color SVG background no CSS
                        // color can reach — swap it for the visually identical
                        // dashicons gear, which follows the item's text color
                        // natively (rest, hover, front) like every core icon.
                        $node = $bar->get_node('aioseo-main');
                        if ($node !== null) {
                            $args = get_object_vars($node);
                            if (is_string($args['title'] ?? null)) {
                                $args['title'] = (string) preg_replace(
                                    '/<div class="ab-item aioseo-logo svg"><\/div>/',
                                    '<span class="ab-icon dashicons-before dashicons-admin-generic"></span>',
                                    $args['title'],
                                );
                                $bar->add_node($args);
                            }
