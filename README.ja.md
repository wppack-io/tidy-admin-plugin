...
導線の外へ出し、WordPress 標準の UI に沿った一貫した場所 — 各プラグイン自身の
画面の「アップグレード」「ヘルプ」ボタンと、ダッシュボードの「セットアップが
必要なプラグイン」ウィジェット — に移設します。機能は何も削除しません。すべての
ページとリンクは引き続き到達可能で、整理内容はプラグインごと・機能ごとに
オン/オフできます。
