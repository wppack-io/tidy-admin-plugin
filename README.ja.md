...
**セットアップが必要なプラグイン**ウィジェット1か所へ。さらに、ベンダーが
wp-admin に押し付けた見た目 — ブランド色のメニューアイコン、標準外の
ツールバーのホバー、不揃いな通知バッジ — を WordPress 標準のスタイルへ
正規化し、プラグインの管理バーメニューごと非表示にすることもできます。
機能は何も削除しません。すべてのページとリンクは引き続き到達可能で、
整理内容はプラグインごと・機能ごとにオン/オフできます。
...
- **設定 › Tidy Admin** で、プラグインごとに整理を無効化できます — 全体でも、
  ...
  列挙します。すべて標準でオンです。
- **プラグイン › Plugin Upgrades**(および同内容のダッシュボードウィジェット)は、
  有効な各プラグインの Pro アップグレードリンクを 1 つのコンパクトなカード一覧に
  まとめます — リンクと短いプロモだけなので、プラグインごとに催促されることなく
  一箇所でアップグレードを見つけられます。プレミアム機能やドキュメントの詳細は、
  各プラグインの Help / Upgrades パネルに残ります。
...
| [Elementor](https://wordpress.org/plugins/elementor/) | 4.x |
...
| [TaxoPress](https://wordpress.org/plugins/simple-tags/) | 3.x |
...
| [WPForms Lite](https://wordpress.org/plugins/wpforms-lite/) | 1.x, 2.x |
